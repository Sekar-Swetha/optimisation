...
\caption{Q4A: Convergence comparison on Benchmark B (note Nesterov random search runs for 220 iterations per the assignment spec, while the GD variants run for 120). Exact GD converges fastest and most accurately ($f = 0.7244$ by iteration $\sim$30). FD GD with $\delta = 0.05$ closely tracks it and reaches $f = 0.7280$, confirming the gradient approximation is essentially unbiased at this scale. FD GD with $\delta = 0.8$, however, does not simply ``converge poorly'' --- it visibly \textbf{stalls/plateaus} at $f \approx 1.6402$ after $\sim$25 iterations and never escapes: the large perturbation $\delta$ makes the forward-difference estimate a systematically \emph{biased} approximation of $\nabla f$ (the secant slope over $[x, x+\delta e_i]$ differs substantially from the true derivative at $x$ when curvature is significant over that interval), so the iterate converges to a stationary point of a \emph{distorted} objective rather than of $f$ itself. \textbf{Nesterov random search}, by contrast, is the noisiest and (per-iteration) the slowest to progress --- each step uses only a single random direction, giving a high-variance, low-information update --- but its directional-derivative estimator is unbiased in expectation (for $u_k$ uniform on the unit sphere, $\mathbb{E}[\hat g_k] = \nabla f(x_k)/d$), so it keeps making slow, noisy progress \emph{towards the true minimum} rather than stalling: by its 220th iteration it reaches $f = 0.7262$, \emph{better} than the converged plateau of poor FD GD despite starting from a far noisier per-step estimate. The lesson is that a biased low-variance estimator (large-$\delta$ FD) can be worse in the long run than an unbiased high-variance one (random search): bias creates a hard floor that more iterations cannot overcome, whereas variance can be averaged away over time.}
\label{fig:q4_fval_B}
\end{figure}
